...
function template_main()
{
	...
	echo'<div id="display_head" class="information">
			...
			<span class="smalltext">(', $txt['read'], ' ', $context['num_views'], ' ', $txt['times'], ')</span>';
		...
		echo '
			<div class="panel panel-default">
				<div class="panel-heading">
						<span class="text-left"><i class="fa fa-pie-chart"></i> ', $context['poll']['question'], '</span>
				</div>
		...
		foreach ($context['linked_calendar_events'] as $event)
			echo '
							<li class="list-group-item">
								', ($event['can_edit'] ? '<a href="' . $event['modify_href'] . '"><i class="fa fa-calendar"></i> </a> ' : ''), '<strong>', $event['title'], '</strong>: ', $event['start_date'], ($event['start_date'] != $event['end_date'] ? ' - ' . $event['end_date'] : ''), '
							</li>';
		echo '
						</ul>
			</div>';
	...
	echo '
			<div class="pagesection">
				...
				<div class="pagelinks floatleft">', !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . '<a  class="button floatleft" href="#lastPost"><strong>' . $txt['go_down'] . '</strong></a> &nbsp;&nbsp;' : '', $txt['pages'], ': ', $context['page_index'], '</div>
			</div>';
	...
	while ($message = $context['get_message']())
	{
		...
		if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
			echo '
								', $context['can_send_pm'] ? '<a href="' . $message['member']['online']['href'] . '" title="' . $message['member']['online']['label'] . '">' : '', '<i class="fa fa-' . ($message['member']['online']['is_online'] ? 'smile-o' : 'frown-o') . '" title="', $message['member']['online']['text'], '" ></i>', $context['can_send_pm'] ? '</a>' : '';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				...
				echo '	
								<li class="avatar">
									<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
										<img class="avatar"  src="'.$settings['images_url'].'/noavatar.png" alt="*"/>
									</a>
								</li>';
			...
			if ($settings['show_profile_buttons'])
			{
				...
				if ($message['member']['can_view_profile'])
					echo '
										<li><a href="', $message['member']['href'], '">', ($settings['use_image_buttons'] ? '<i class="fa fa-profile" title="' . $txt['view_profile'] . '"></i>' : $txt['view_profile']), '</a></li>';

				// Don't show an icon if they haven't specified a website.
				if ($message['member']['website']['url'] != '' && !isset($context['disabled_fields']['website']))
					echo '
										<li><a href="', $message['member']['website']['url'], '" title="' . $message['member']['website']['title'] . '" target="_blank" class="new_win">', ($settings['use_image_buttons'] ? '<i class="fa fa-globe"></i>' : $txt['www']), '</a></li>';

				// Don't show the email address if they want it hidden.
				if (in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
					echo '
										<li><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<i class="fa fa-envelope" title="' . $txt['email'] . '"></i>' : $txt['email']), '</a></li>';

				// Since we know this person isn't a guest, you *can* message them.
				if ($context['can_send_pm'])
					echo '
										<li><a href="', $scripturl, '?action=pm;sa=send;u=', $message['member']['id'], '" title="', $message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline'], '">', $settings['use_image_buttons'] ? '<i class="fa fa-' . ($message['member']['online']['is_online'] ? 'comment' : 'comment-o') . '" title="' . ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']) . '" ></i>' : ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']), '</a></li>';
		...
		elseif (!empty($message['member']['email']) && in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
			echo '
								<li class="email"><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<i class="fa fa-envelope" title="' . $txt['email'] . '" ></i> ' : $txt['email']), '</a></li>';
		...
		if (!empty($message['attachment']))
		{
			...
			foreach ($message['attachment'] as $attachment)
			{
				...
				echo '
										<a href="' . $attachment['href'] . '"><i class="fa fa-paperclip"></i>&nbsp;' . $attachment['name'] . '</a> ';
...
// Maybe we can approve it, maybe we should?
					if ($message['can_approve'])
						echo '
												<li class="approve_button"><a href="', $scripturl, '?action=moderate;area=postmod;sa=approve;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><i class="fa fa-thumbs-o-up"></i> ', $txt['approve'], '</a></li>';

					// Can they reply? Have they turned on quick reply?
					if ($context['can_quote'] && !empty($options['display_quick_reply']))
						echo '
												<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');"><i class="fa fa-quote-right"></i> ', $txt['quote'], '</a></li>';

					// So... quick reply is off, but they *can* reply?
					elseif ($context['can_quote'])
						echo '
												<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '"><i class="fa fa-quote-right"></i> ', $txt['quote'], '</a></li>';

					// Can the user modify the contents of this post?
					if ($message['can_modify'])
						echo '
												<li class="modify_button"><a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '"><i class="fa fa-pencil-square-o"></i> ', $txt['modify'], '</a></li>';

					// How about... even... remove it entirely?!
					if ($message['can_remove'])
						echo '
												<li class="remove_button"><a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');"><i class="fa fa-trash"></i> ', $txt['remove'], '</a></li>';

					// What about splitting it off the rest of the topic?
					if ($context['can_split'] && !empty($context['real_num_replies']))
						echo '
												<li class="split_button"><a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '"><i class="fa fa-columns"></i> ', $txt['split'], '</a></li>';

					// Can we restore topics?
					if ($context['can_restore_msg'])
						echo '
												<li class="restore_button"><a href="', $scripturl, '?action=restoretopic;msgs=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><i class="fa fa-window-restore "></i> ', $txt['restore_message'], '</a></li>';
	...
	echo '
		...
		if ($settings['show_modify'] && !empty($message['modified']['name']))
			echo '
								<span class="smalltext modified">', $txt['last_edit'], ': ', $message['modified']['time'], ' ', $txt['by'], ' ', $message['modified']['name'], '</span>';

		echo '
							</div>
							<div class="smalltext reportlinks  floatright">';
		// Can the user modify the contents of this post?  Show the modify inline image.
		if ($message['can_modify'])
			echo '
							<i class="fa fa-pencil-square-o" id="modify_button_', $message['id'], '" style="cursor: ', ($context['browser']['is_ie5'] || $context['browser']['is_ie5.5'] ? 'hand' : 'pointer'), '; " onclick="oQuickModify.modifyMsg(\'', $message['id'], '\')" ></i>';
		// Maybe they want to report this post to the moderator(s)?
				if ($context['can_report_moderator'])
				echo '
								<a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '" title="', $txt['report_to_mod'], '"><i class="fa fa-exclamation-triangle"></i></a>';
				if ($context['can_issue_warning'] && !$message['is_message_author'] && !$message['member']['is_guest'])
					echo '
										<a href="', $scripturl, '?action=profile;area=issuewarning;u=', $message['member']['id'], ';msg=', $message['id'], '"><i class="fa fa-exclamation-triangle"></i></a>';
				
				if ($context['can_moderate_forum'] && !empty($message['member']['ip']))
					echo '
										<a href="', $scripturl, '?action=', !empty($message['member']['is_guest']) ? 'trackip' : 'profile;area=tracking;sa=ip;u=' . $message['member']['id'], ';searchip=', $message['member']['ip'], '" title="', $message['member']['ip'], '"><i class="fa fa-server"></i></a> <a href="', $scripturl, '?action=helpadmin;help=see_admin_ip" onclick="return reqWin(this.href);" class="help"><i class="fa fa-question-circle"></i></a>';
				elseif ($message['can_see_ip'])
					echo '
										<a href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help" title="', $message['member']['ip'], '"><i class="fa fa-server"></i></a>';
				elseif (!$context['user']['is_guest'])
					echo '
										<a href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help" title="', $txt['logged'], '"><i class="fa fa-server"></i></a>';
				else
					echo '<i class="fa fa-server"></i>', $txt['logged'];
	...
	echo '
			<div class="pagesection">
				', template_button_strip($normal_buttons, 'right'), '
				<div class="pagelinks floatleft">', !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . '<a  class="button floatleft" href="#top"><strong>' . $txt['go_up'] . '</strong></a> &nbsp;&nbsp;' : '', $txt['pages'], ': ', $context['page_index'], '</div>
			</div>';
	...
	if ($context['can_reply'] && !empty($options['display_quick_reply']))
	{
		...
		echo '<script async="" type="text/javascript" src="' . $settings['default_theme_url'] . '/scripts/spellcheck.js?fin20"></script>';	
		echo '<script async="" type="text/javascript" src="' . $settings['default_theme_url'] . '/scripts/editor.js?fin20"></script>
			<a id="quickreply"></a>
