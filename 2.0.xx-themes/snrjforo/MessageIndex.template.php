...
function template_main()
{
	...
	<div class="catmain" id="board_', $context['current_board'], '_childboards">
		...
		foreach ($context['boards'] as $board)
		{
			echo '<div class="boardbg"><div class="board_icon">';
				if ($board['new'])
					echo '	<a href="', ($board['is_redirect'] || $context['user']['is_guest'] ? $board['href'] : $scripturl . '?action=unread;board=' . $board['id'] . '.0'), '" >';
	...
	if (!empty($options['show_board_desc']) && $context['description'] != '')
		echo '
	<p class="alert alert-info">', $context['description'], '</p>';

	// Create the button set...
	$normal_buttons = array(
		'sort_up' => array('test' => 'can_post_new', 'text' => 'new_topic', 'image' => 'new_topic.gif', 'lang' => true, 'url' => $scripturl . '?action=post;board=' . $context['current_board'] . '.0', 'active' => true),
		'poll' => array('test' => 'can_post_poll', 'text' => 'new_poll', 'image' => 'new_poll.gif', 'lang' => true, 'url' => $scripturl . '?action=post;board=' . $context['current_board'] . '.0;poll'),
		...
		'unread_button' => array('text' => 'mark_read_short', 'image' => 'markread.gif', 'lang' => true, 'url' => $scripturl . '?action=markasread;sa=board;board=' . $context['current_board'] . '.0;' . $context['session_var'] . '=' . $context['session_id']),
	);
	...
	<div class="pagelinks pull-left"><ul class="pagination pagination-sm">',$veli,'</ul></div>
		...
		if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] > 0 && !empty($context['topics']))
			echo '
	<form action="', $scripturl, '?action=quickmod;board=', $context['current_board'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" class="clearfix" name="quickModForm" id="quickModForm">';

		echo '
		<div class="boardindextable clear">';
	if (!empty($settings['display_who_viewing']))
		...
		foreach ($context['topics'] as $topic)
		{
			...
			if ($context['can_approve_posts'] && $topic['unapproved_posts'])
				$color_class = !$topic['approved'] ? '<span class="generic_icons unapprove_button pull-right"></span>' : '<span class="generic_icons approve pull-right"></span>';
			// We start with locked and sticky topics.
			elseif ($topic['is_sticky'] && $topic['is_locked'])
				$color_class = '<span class="generic_icons sticky pull-right"></span><br/><span class="generic_icons lock pull-right"></span>';
			// Sticky topics should get a different color, too.
			elseif ($topic['is_sticky'])
				$color_class = '<span class="generic_icons sticky pull-right"></span>';
			// Locked topics get special treatment as well.
			elseif ($topic['is_locked'])
				$color_class = '<span class="generic_icons lock pull-right"></span>';
			...
			echo '
				<div class="boardbg">
					...
					loadMemberContext($topic['first_post']['member']['id']);
							if($memberContext[$topic['first_post']['member']['id']]['avatar']['image'])
				echo'', $memberContext[$topic['first_post']['member']['id']]['avatar']['image'],'';
				else
				echo'<img class="avatar" src="'.$settings['images_url'].'/theme/default_avatar.png" alt="*" />';
					echo'<img class="topicon" src="', $topic['first_post']['icon_url'], '" alt="*" />
					</div>
					<div class="info">
			...
			echo '
					...
					<div class="board_stats text-center">
		...
		{
			echo '<div class="haberler text-right">
			...
			echo '
						<input type="submit" value="', $txt['quick_mod_go'], '" onclick="return document.forms.quickModForm.qaction.value != \'\' &amp;&amp; confirm(\'', $txt['quickmod_confirm'], '\');" class="btn btn-sm btn-success qaction" />
