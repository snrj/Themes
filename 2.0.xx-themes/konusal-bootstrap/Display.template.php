...
function template_main()
{
	...
	echo '
			<div id="forumposts">
				<div class="well well-sm">
						...
						<strong>', $context['subject'], '</strong><span class="floatright">(', $txt['read'], ' ', $context['num_views'], ' ', $txt['times'], ')</span>
				</div>';
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg' : 'windowbg2') : 'approvebg', '  drop-shadow curled">
					<div class="post_wrapper"><div class="col-sm-3 posbit">';
			...
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				...
				echo '
					<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal', $message['member']['id'], '">', $message['member']['name'], '</button></div>
		...
		echo '<div class="modal fade" id="myModal', $message['member']['id'], '" role="dialog">
			...
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				{echo '', $message['member']['avatar']['image'], '';}
						...
						echo'	<ul class="smalltext list-group" id="msg_', $message['id'], '_extra_info">
						<li class="list-group-item">',$txt['pkullan'],'<span class="floatright">';

		// Show online and offline buttons?',$txt['pkullan'],'
		...
		if (!empty($message['member']['title']))
			echo '
								<li class="title list-group-item">',$txt['pkullanb'],'<span class="floatright">', $message['member']['title'], '</span></li>';

		// Show the member's primary group (like 'Administrator') if they have one.
		if (!empty($message['member']['group']))
			echo '
								<li class="membergroup list-group-item">',$txt['pkullany'],'<span class="floatright">', $message['member']['group'], '</span></li>';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
				echo '
								<li class="postgroup list-group-item">',$txt['pkullang'],'<span class="floatright">', $message['member']['post_group'], '</span></li>';
			echo '
								<li class="stars list-group-item">',$txt['pkullanr'],'<span class="floatright">', $message['member']['group_stars'], '</span></li>';
			...
			if (!isset($context['disabled_fields']['posts']))
				echo '
								<li class="postcount list-group-item"><span class="label label-default">', $txt['member_postcount'], ':</span> <span class="floatright">', $message['member']['posts'], '</span></li>';

			// Is karma display enabled?  Total or +/-?
			if ($modSettings['karmaMode'] == '1')
				echo '
								<li class="karma list-group-item"><span class="label label-warning">', $modSettings['karmaLabel'], '</span> <span class="floatright">', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</span></li>';
			...
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
				echo '
								<li class="gender list-group-item"><span class="label label-info">', $txt['gender'], ': </span><span class="floatright">', $message['member']['gender']['image'], '</span></li>';

			// Show their personal text?
			if (!empty($settings['show_blurb']) && $message['member']['blurb'] != '')
				echo '
								<li class="blurb list-group-item">',$txt['pkullank'],'<span class="floatright">', $message['member']['blurb'], '</span></li>';
		...
		elseif (!empty($message['member']['email']) && in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
			echo '
								<li class="email list-group-item"><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/email_sm.gif" alt="' . $txt['email'] . '" title="' . $txt['email'] . '" />' : $txt['email']), '</a></li>';
	...
	if ($context['can_reply'] && !empty($options['display_quick_reply']))
	{
		echo '
			<a id="quickreply"></a>
			<div class="panel panel-info tborder" id="quickreplybox">
