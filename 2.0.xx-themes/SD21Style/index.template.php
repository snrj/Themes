...
if ($context['user']['is_logged'])
	...
	echo'<div class="dropdown">
		...
		<div class="dropdown-content">
					<a href="', $scripturl, '?action=profile"><span class="generic_icons members"></span> ', $txt['profile'], '</a>
					<a href="', $scripturl, '?action=profile;area=account"><span class="generic_icons members_request"></span> ', $txt['account'], '</a>
					<a href="', $scripturl, '?action=profile;area=forumprofile"><span class="generic_icons starters"></span> ', $txt['forumprofile'], '</a>
					<a href="', $scripturl, '?action=pm"><span class="generic_icons pm"></span> 
					', $txt['pm_short'], '
					', !empty($context['user']['unread_messages']) ? '<strong>'.$context['user']['unread_messages'] .'</strong>' : '', '
					</a>
					<a href="', $scripturl, '?action=unread"><span class="generic_icons replies"></span> ', $txt['unread_since_visit'], '</a>
					<a href="', $scripturl, '?action=unreadreplies"><span class="generic_icons drafts"></span>  ', $txt['show_unread_replies'], '</a>';
		if ($context['in_maintenance'] && $context['user']['is_admin'])
			echo '
					', $txt['maintain_mode_on'], '';
		if (!empty($context['unapproved_members']))
			echo '
					', $context['unapproved_members'] == 1 ? $txt['approve_thereis'] : $txt['approve_thereare'], ' <a href="', $scripturl, '?action=admin;area=viewmembers;sa=browse;type=approve"><span class="generic_icons approve"></span> ', $context['unapproved_members'] == 1 ? $txt['approve_member'] : $context['unapproved_members'] . ' ' . $txt['approve_members'], '</a> ', $txt['approve_members_waiting'], '';

		if (!empty($context['open_mod_reports']) && $context['show_open_reports'])
			echo '
					<a href="', $scripturl, '?action=moderate;area=reports"><span class="generic_icons warning_moderate"></span>  ', sprintf($txt['mod_reports_waiting'], $context['open_mod_reports']), '</a>';
		echo '
		</div>
	</div>';
	}
	elseif (!empty($context['show_login_bar']))
	{
		echo '<div class="buttonlist">
				<ul>
					<li>
						<a class="button" id="giris"><span><i class="generic_icons login"></i> ',$txt['login'],'</span></a> 
					</li>	
					<li>
						<a class="button" href="' . $scripturl . '?action=register"><span><i class="generic_icons register"></i> ',$txt['register'],'</span></a>
					</li>	
				</ul>
			</div>
